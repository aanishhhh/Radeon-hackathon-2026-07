...
def main() -> None:
    ...
    image = bundle.world_cam.render(rgb=True)[0]

    result = inspection_manager.inspect_pcb(image)

    print("\n========== RoboAOI Inspection ==========")
    print("Status:", result["status"])
    print("Components Detected:", len(result["components"]))
    print("PCB Defects:", len(result["pcb_defects"]))

    if result["status"] == "PASS":

        print("\nPCB PASSED inspection.")
        print("Starting robot pick-and-place...\n")
        ...
        print(
            f"[grasp_demo] task: pick={task.pick_object} "
            f"place={task.place_target} -> success = {success}"
        )

    else:

        print("\nPCB FAILED inspection.")
        print("Robot action cancelled.")
